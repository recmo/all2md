...
def test_completion_retries_identical_bytes_after_lost_response(monkeypatch):
    ...
    assert client.request("/worker", {"op": "complete", "id": "job", "attempt": "lease", "outputs": {}}) is None
...
def test_worker_does_not_retry_non_idempotent_claim(monkeypatch):
    ...
    with pytest.raises(URLError):
        worker.Client("http://localhost", "worker-token").request("/worker", {"op": "claim", "recipes": []})
    assert len(calls) == 1


@pytest.mark.parametrize("failed", [False, True])
def test_worker_vector_search_preserves_lease(monkeypatch, failed):
    import io
    ...
    def urlopen(request, **kwargs):
        assert request.full_url == "http://localhost/worker"
        assert request.get_header("Authorization") == "Bearer worker-token"
        vector_query = {k: v for k, v in query.items() if k not in ("job", "attempt")}
        assert json.loads(request.data) == {"op": "search", "id": "job", "attempt": "lease", "query": vector_query}
        if failed:
            raise HTTPError(request.full_url, 422, "Unprocessable Entity", {}, io.BytesIO(b'{"detail":"lease expired"}'))
        return io.BytesIO(b'{"results":[{"id":"match"}]}')
