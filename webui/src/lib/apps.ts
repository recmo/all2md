...
export function appDocuments(cache: Cache) {
  ...
  for (const path of paths) {
    ...
    try {
      ...
      documents.push({ path, title: title || path.split('/').at(-1)!, template: templates[0] ? '/' + templates[0] : null, frontmatter, text: page?.text ?? null });
...
export async function evaluateApp(path: string, source: string, documents: AppDocument[], action?: string, event?: unknown, signal?: AbortSignal): Promise<AppResult & { edits?: AppEdit[] }> {
  ...
  return new Promise((resolve, reject) => {
    ...
    try { worker.postMessage({ wasmUrl, input: { path, source, documents, action, event } }); }
