<script lang="ts">
  ...
  function arrow(source: string, target: string) {
    const a = layout.positions.get(source)!,
      b = layout.positions.get(target)!;
    const x1 = (a.to - layout.from) * unit,
      x2 = (b.from - layout.from) * unit;
    const y1 = a.lane * 36 + 15,
      y2 = (b.row - a.row) * layout.cellHeight + b.lane * 36 + 15;
...
<svg class="dependencies" aria-label="Task dependencies">
  {#each layout.links.filter( (link) => row.bars.some((bar) => bar.id === link.source) ) as link}
    ...
    {@const y =
      (target.row - data.rowIndex) * layout.cellHeight + target.lane * 36 + 15}
