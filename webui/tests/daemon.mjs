...
writeFileSync(
  join(root, 'reciprocal', 'template.md'),
...
writeFileSync(join(root, 'tasks/v1/template.md'), readFileSync(new URL('../../mdstore/examples/tasks/v1/template.md', import.meta.url), 'utf8') + '\n```starlark\nscope(exclude=["app.md"])\n```\n');
...
const ui = await preview({ preview: { host: '127.0.0.1', port: 43132, strictPort: true } });
