...
use starlark::{
    ...
    values::{list::AllocList, structs::AllocStruct},
};
...
pub(crate) fn is_app(text: &str) -> bool {
    mdstore::validation::parse_frontmatter(text)
        .is_ok_and(|(value, _, _)| value["mdstore"] == "app")
}
...
fn run(input: Input) -> Result<Json> {
    ...
    Module::with_temp_heap(|module| {
        ...
        for (path, text) in [
            ("<mdstore-app>", include_str!("app_prelude.star").to_owned()),
            (input.path.as_str(), source),
        ] {
            let ast = AstModule::parse(path, text, &Dialect::Standard)
        ...
        let runner = module
            .get("mdstore_app_run")
            .context("missing app runner")?;
        let args = [
            documents,
            input
                .action
                .as_ref()
                .map_or(starlark::values::Value::new_none(), |v| heap.alloc(v)),
            input
                .event
                .as_ref()
                .map_or(starlark::values::Value::new_none(), |v| heap.alloc(v)),
        ...
        let result = eval
            .eval_function(runner, &args, &[])
            .map_err(|e| anyhow::anyhow!("{e}"))?
            .to_json()?;
        if result.len() > 8 * 1024 * 1024 {
            bail!("app result exceeds 8 MiB");
        }
...
mod tests {
    ...
    fn run(source: &str) -> Result<Json> {
        evaluate(Input {
            path: "app.md".into(),
            source: format!("```starlark\n{source}\n```\n"),
            documents: vec![
                serde_json::json!({"path":"a.md", "title":"A", "template":"schema.md", "frontmatter":{"state":"ready"}}),
            ],
            action: None,
            event: None,
        })
    ...
    fn gantt_grouping_is_an_optional_field_binding() {
        ...
        assert_eq!(
            result["views"][1]["bindings"]["dependencies"],
            "/depends_on"
        );
    ...
    fn collection_inputs_are_immutable_and_bindings_checked() {
        ...
        assert!(
            run("collection('all', lambda docs: docs)\ntable('Bad', 'all', columns=[42])").is_err()
        );
    }
    #[test]
    ...
    fn task_example_runs_and_action_returns_proposed_edits() {
        let mut input = Input {
            path: "tasks/v1/app.md".into(),
            source: include_str!("../../examples/tasks/v1/app.md").into(),
            documents: vec![
                serde_json::json!({"path":"tasks/v1/a.md", "title":"A", "template":"/tasks/v1/schema.md", "frontmatter":{"state":"inbox"}, "text":null}),
            ],
            action: None,
            event: None,
        };
        ...
        input.event = Some(
            serde_json::json!({"path":"tasks/v1/a.md","value":"ready","timestamp":"2026-09-23T10:00:00Z"}),
        );
        assert_eq!(
            evaluate(input).unwrap()["edits"][0]["fields"]["state"],
            "ready"
        );
    }
}
