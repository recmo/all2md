...
const doc = (path: string, frontmatter: Record<string, unknown>): AppDocument => ({path, title: path, frontmatter, template: null, text: null});
...
it('keeps unassigned tasks distinct and reports invalid or missing dates', () => {
  const model = timelineData([
    ...
    doc('b.md', {who:'Unassigned', from:'2026-09-23', through:'2026-09-23'}),
    doc('bad.md', {who:'Alice', from:'2026-02-30', through:'2026-03-01'}),
    doc('backwards.md', {who:'Alice', from:'2026-09-25', through:'2026-09-23'}),
    doc('missing.md', {who:'Alice'}),
  ], '/who', '/from', '/through');
  expect(model.items).toHaveLength(2);
  expect(model.items[0].group).not.toBe(model.items[1].group);
  expect(model.unscheduled.map(d => d.path)).toEqual(['bad.md','backwards.md','missing.md']);
});
...
it('ungrouped timelines give each task its own row in collection order', () => {
  ...
  expect(model.groups.map(g => g.id)).toEqual(['document:z.md','document:a.md']);
  expect(model.items.map(i => i.group)).toEqual(model.groups.map(g => g.id));
});
...
it('packs lanes, reuses lanes after inclusive end dates, and resolves path dependencies', async () => {
  ...
  const model = timelineData([
    doc('a.md', {who:'Alice',from:'2026-09-23',through:'2026-09-25'}),
    doc('b.md', {who:'Alice',from:'2026-09-24',through:'2026-09-24',deps:['/a.md']}),
    doc('c.md', {who:'Alice',from:'2026-09-26',through:'2026-09-26',deps:['b.md','b.md']}),
    doc('d.md', {who:'Bob',from:'2026-09-26',through:'2026-09-27',deps:['c.md','missing.md']}),
  ], '/who', '/from', '/through');
  ...
  expect(layout.links).toEqual([{source:'a.md',target:'b.md'},{source:'b.md',target:'c.md'},{source:'c.md',target:'d.md'}]);
...
it('moves calendar dates across DST and clamps either resized edge to a single day', async () => {
  ...
  const from = calendarDay(new Date(2026,9,24)), to = calendarDay(new Date(2026,9,27));
