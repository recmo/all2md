    ...
    *,
    ...
    assembly_fingerprint = {
        ...
        "code": _code_fingerprint(
            ...
            "quality.py", "decoding.py", "region_recovery.py", "verify.py", "syntax.py", "reconciliation.py", "document.py", "edits.py", "mathlint.py", "katex_lint.cjs", "urls.py", "semantics.py", "footnotes.py",
        ),
    }
...
def _collect_page_candidates(
    ...
    from .region_recovery import collect_regions
    try:
...
def _restore_embedded_proof_marks(
    ...
    for glyph in _embedded_characters(embedded, EMBEDDED_PROOF_MARKS):
        ...
        overlapping = [
            (index, block)
            for index, block in enumerate(blocks)
            if block.bbox and bbox_coverage(glyph["bbox"], block.bbox) >= 0.45
        ]
        existing = next(
            (
                block
        ...
        if any(
            ...
            and block.bbox
            and bbox_coverage(glyph["bbox"], block.bbox) >= 0.45
            for block in blocks
        ):
            continue
        proof = Block(
