...
def standalone_math(block):
    ...
    if outside.strip(" .,:;\n\t"):
        return None
    body = block.markdown[span.content_start:span.content_end].strip()
    # A single math span can contain multiple equations or matrix rows.
    if r"\\" in body or re.search(r"\\(?:cr|newline)\b", body):
        return None
    return body
